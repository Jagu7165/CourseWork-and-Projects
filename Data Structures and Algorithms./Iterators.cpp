...
private:
  ...
  bool pastTheEnd;
  shared_ptr<Node<T>> nodePtr{ nullptr };
...
bool Iterator<T>::operator!=(const Iterator<T>& rhs) const{
  if(this->nodePtr->data == rhs.nodePtr->data && this->pastTheEnd == rhs.pastTheEnd){
    return false;
  }else{
    return true;
  }
...
template <typename T>
Iterator<T> Iterator<T>::operator--() {
  if(this->pastTheEnd == true){
    this->pastTheEnd=false;
  }
  else if (this->nodePtr->next== nullptr & this->nodePtr->prev == nullptr){
    cout<<"nope"<<endl;
  }
  else{
