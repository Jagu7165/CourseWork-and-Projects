...
void step1() {
    ...
    // then assign that array value into that bucket.kl

    //loop through the array and assign each value into each bucket
    for (int i = 0; i < arrSize; i++) {
        unsigned int target = 0;
        if (numBuckets == 1) {
            target = 0;
            buckets[target].push_back(arr[i]);
        }
        else {
            unsigned int target = (arr[i] / ((UINTMAX / numBuckets) + 1));
            buckets[target].push_back(arr[i]);
        }
    }
...
void step2() {
    ...
    unsigned int theBucket = 0;

    while (true) {
        myMutex.lock();
        theBucket = currentBucket;
        currentBucket++;
        myMutex.unlock();
        if (theBucket >= numBuckets) {
            break;
        }
        sortOneVector(buckets[theBucket]);
    }
}
...
void step3() {
    ...
    // Index of which bucket you are working with bucket[]
    ...
    for (int i = 0; i <= numBuckets - 1; i++)
    {
        ...
        for (int j = 0; j <= (buckets[i].size() - 1); j++) {
            arr[arrayIndex] = (buckets[i].at(j));
            arrayIndex++;
        }
        // std::copy(buckets[i].begin(), buckets[i].end(), arr[arrayIndex]);
        // arrayIndex++;
    }
...
void bucketSort() {
    ...
    //create array of thread tracking objects
    ...
    //join
    for (unsigned int z = 0; z < numThreads; z++) {
        threads[z].join();
    }
    delete[] threads;
