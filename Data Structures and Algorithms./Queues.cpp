...
template <typename T>
QueueForCS2420<T>::QueueForCS2420(const unsigned int capacity) {                     //constructor
  // TODO: Complete this
...
template <typename T>
QueueForCS2420<T>::~QueueForCS2420() {                                               //destructor
  if (arr != nullptr) {
    delete[] arr;
  }
}
...
template <typename T>
unsigned int QueueForCS2420<T>:: size() {                                            //size
  return index;
}

template <typename T>
void QueueForCS2420<T>::push_back(const T& data) {                                   //puch_back
  if(this->index==this->cap){
    cout<<"nah were full"<<endl;
  }
  else{
    this->arr[tail] = data;
    this->tail++;
...
template <typename T>
void QueueForCS2420<T>::pop_front() {                                             //pop_front
  if(this->index==0){
...
template <typename T>
T QueueForCS2420<T>::front() {                                                      //front
  if(this->index==0){
    cout<<"nope nothing"<<endl;
    throw 1;
  }
...
template <typename T>
void QueueForCS2420<T>::resize( const unsigned int newCapacity) {                  //resize

  QueueForCS2420<T> newQueue(newCapacity);
  while((this->index)!=0){
    newQueue.push_back(this->front());
    this->pop_front();
  }
  delete[] this->arr;
  this->cap = newQueue.cap;
  this->index = newQueue.index;
  this->head = newQueue.head;
  this->tail = newQueue.tail;
  this->arr = newQueue.arr;
  newQueue.arr=nullptr;
}
...
//Copyright 2021, Bradley Peterson, Weber State University, All rights reserved. (May 2021)
