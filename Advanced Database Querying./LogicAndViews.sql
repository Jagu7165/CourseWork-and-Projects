...
-------------------------------------------------------------------------------------------------------------------------------------------------------
----------create new table gamOfficialPaymentHistory
-----------------------------------------------------------------------------------------------------------------------------------------------------------
Create Table gosGameOfficialPaymentHistory(
	odPaymentHistory_id INT not null identity(1,1) ,
	odUser_id INT not null ,
...
Primary Key Clustered (odUser_id, odGame_id, odPositionName_id);
go
...
Alter Table gosGameOfficial
add constraint FK_gosGameOfficial_odPosition_id_
foreign key (odPositionName_id) references gosOfficiatingPosition (odPositionName_id);

-----------------------------------------------------------------------------------------------------------------------------------------------------------
--------------foreign keys for gosGameOfficialPaymentHistory
----------------------------------------------------------------------------------------------------------------------------------------------------

Alter Table gosGameOfficialPaymentHistory
add constraint FK_gosGameOfficialPaymentHistory_odGame_id_
foreign key (odGame_id) references gosGame (odGame_id);

Alter Table gosGameOfficialPaymentHistory
add constraint FK_gosGameOfficialPaymentHistory_odUser_id_
foreign key (odUser_id) references gosUser (odUser_id);

Alter Table gosGameOfficialPaymentHistory
add constraint FK_gosGameOfficialPaymentHistory_odPositionName_id_
foreign key (odPositionName_id) references gosOfficiatingPosition (odPositionName_id);
--------------------------------------------------------------
Alter Table gosArbiter
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------
-----------------alternate key for gosGameOfficialPaymentHistory
----------------------------------------------------------------------------------------------------------------------------------------------------------


Alter Table gosGameOfficialPaymentHistory
Add Constraint AK_GameOfficialPaymentHistory_User_Game_Position
Unique(odUser_id, odGame_id);
...
from gosGame gg
where( GameDateTime  = @GameDateTime
and gg.odSite_id = (select odSite_id from gosSite where SiteName = @GameSite)
and gg.odSportLevel_id = (Select odSportLevel_id from gosSportLevel where Sport = @Sport and [Level] = @Level));
...
begin
	begin try
		...
		From gosGameOfficial ggo
		Inner Join gosGame gg
	...
	begin catch
		print 'insert into gosGameOfficial failed for ' + convert(nvarchar, ([dbo].udf_getGameID(convert(datetime, @GameDateTime),@GameSite, @Sport, @Level))) + ' ' +@GameDateTime + ' ' + @PositionName + ' ' + @Sport
	end catch
end
go
...
-------------------------------------------------------------------------------------------------------------------------------------------------------
-----------------create udp_GameOfficialPayment procedure
----------------------------------------------------------------------------------------------------------------------------------------------------------
if exists (select * from INFORMATION_SCHEMA.ROUTINES where SPECIFIC_NAME = 'usp_addGameOfficialPayment')
drop procedure usp_addGameOfficialPayment;

go

create procedure [dbo].usp_addGameOfficialPayment
  @EmailAddress nvarchar(255),
  @datetime dateTime,
  @SiteName nvarchar(255),
  @Sport nvarchar(10),
...
begin
	begin try
		declare
		@currentFeesAcum money,
		@gameFeePayment money,
		@newFeesAcum money

		set @currentFeesAcum = (select FeesAccumulated from gosOfficial where odUser_id = 
									(Select odUser_id from gosUser where emailAddress = @EmailAddress) ) 
		set @gameFeePayment  = (select PayRate from gosSportLevel where Sport = @Sport and [Level] = @Level)
		set @newFeesAcum =( @currentFeesAcum + @gameFeePayment)

		if not exists
		(select odPaymentHistory_id from gosGameOfficialPaymentHistory where 
			odUser_id = ([dbo].udf_getUserID(@emailAddress)) and
			odGame_id =  ([dbo].udf_getGameID(@datetime,@SiteName, @Sport, @Level)))
		update gosOfficial
		set FeesAccumulated = @newFeesAcum
		where odUser_id = ([dbo].udf_getUserID(@emailAddress)) 
		...
		insert into gosGameOfficialPaymentHistory(
			...
			values(
			([dbo].udf_getUserID(@emailAddress)),
			([dbo].udf_getGameID(@datetime,@SiteName, @Sport, @Level)),
			([dbo].udf_getPositionNameID(@Position)),
			@gameFeePayment)
	
	end try
	begin catch
		print 'insert into GameOfficialPaymentHistory failed for ' + @EmailAddress 
	end catch
end
go
...
execute usp_addSportLevel @sport = 	'Volleyball'	,@Level= 	'Junior Varsity'	,@pay = 	47.00	;
execute usp_addSportLevel @sport = 	'Volleyball'	,@Level= 	'Sophomore'	,@pay = 	47.00	;
execute usp_addSportLevel @sport = 	'Volleyball'	,@Level= 	'Freshman'	,@pay = 	47.00	;
...
execute usp_addSportLevel @sport = 	'Baseball'	,@Level= 	'Varsity'	,@pay = 	69.00	;
...
execute usp_addSportLevel @sport = 	'Softball'	,@Level= 	'Junior Varsity'	,@pay = 	54.00	;
execute usp_addSportLevel @sport = 	'Softball'	,@Level= 	'Sophomore'	,@pay = 	54.00	;
execute usp_addSportLevel @sport = 	'Softball'	,@Level= 	'Freshman'	,@pay = 	54.00	;
...
---------------Insert into Arbiter 
--------------------------------------------------------------------------------------------------------------------------------------------------------
insert into gosArbiter (odUser_id, odSportLevel_id)
values (
		...
		(
			(Select odUser_id from gosUser where emailAddress= 'Sheldonb@hotmail.com'), 
			(Select odSportLevel_id from gosSportLevel where Sport = 'Volleyball' and [Level] = 'Sophomore') ),
		(
			(Select odUser_id from gosUser where emailAddress= 'Sheldonb@hotmail.com'), 
			(Select odSportLevel_id from gosSportLevel where Sport = 'Volleyball' and [Level] = 'Freshman') );

insert into gosArbiter (odUser_id, odSportLevel_id)
values (
			(Select odUser_id from gosUser where emailAddress= 'jeffbraun8@gmail.com'), 
			(Select odSportLevel_id from gosSportLevel where Sport = 'Football' and [Level] = 'Varsity') );

insert into gosArbiter (odUser_id, odSportLevel_id)
values
		(
			(Select odUser_id from gosUser where emailAddress= 'jonathankilburn@aol.com'), 
			(Select odSportLevel_id from gosSportLevel where Sport = 'Football' and [Level] = 'Junior Varsity') );

insert into gosArbiter (odUser_id, odSportLevel_id)
values (
			(Select odUser_id from gosUser where emailAddress= 'paulleonard234@yahoo.com'), 
			(Select odSportLevel_id from gosSportLevel where Sport = 'Baseball' and [Level] = 'Varsity') ),
		(
			(Select odUser_id from gosUser where emailAddress= 'paulleonard234@yahoo.com'), 
			(Select odSportLevel_id from gosSportLevel where Sport = 'Baseball' and [Level] = 'Junior Varsity') ),
		(
			(Select odUser_id from gosUser where emailAddress= 'paulleonard234@yahoo.com'), 
			(Select odSportLevel_id from gosSportLevel where Sport = 'Baseball' and [Level] = 'Sophomore') ),
		(
			(Select odUser_id from gosUser where emailAddress= 'paulleonard234@yahoo.com'), 
			(Select odSportLevel_id from gosSportLevel where Sport = 'Baseball' and [Level] = 'Freshman') );
...
execute usp_addGame  @GameDateTime =	'Oct 20, 2021 4:00 PM'	,@GameSIte =	'USU Maverik Stadium'	,@Sport =	'Football'	,@Level =	'Varsity'	,@HomeTeam =	'Ridgeline High School'	,@AwayTeam =	'Sky View High School'	;
execute usp_addGame  @GameDateTime =	'Oct 20, 2021 4:00 PM'	,@GameSIte =	'WSU Stewart Stadium'	,@Sport =	'Football'	,@Level =	'Varsity'	,@HomeTeam =	'Box Elder High School'	,@AwayTeam =	'Farmington High School'	;
execute usp_addGame  @GameDateTime =	'Oct 20, 2021 4:00 PM'	,@GameSIte =	'Fremont High School'	,@Sport =	'Football'	,@Level =	'Varsity'	,@HomeTeam =	'Bear River High School'	,@AwayTeam =	'Morgan High School'	;

go
-----------------------------------------------------------------------------------------------------------------------------------------------------------------
----------assign gameOfficals for playoff games
----------------------------------------------------------------------------------------------------------------------------------------------------------------

execute usp_addGameOfficial @emailAddress =	'adambaxter@yahoo.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'USU Maverik Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Referee'	;
execute usp_addGameOfficial @emailAddress =	'bryanburningham@hotmail.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'USU Maverik Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Umpire'	;
execute usp_addGameOfficial @emailAddress =	'ronbrenk@q.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'USU Maverik Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Head Linesman'	;
execute usp_addGameOfficial @emailAddress =	'mathew.willey@hotmail.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'USU Maverik Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Line Judge'	;
execute usp_addGameOfficial @emailAddress =	'bobcarre@live.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'USU Maverik Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'BackJudge'	;
execute usp_addGameOfficial @emailAddress =	'dfc99@yahoo.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'WSU Stewart Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Referee'	;
execute usp_addGameOfficial @emailAddress =	'Sheldonb@hotmail.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'WSU Stewart Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Umpire'	;
execute usp_addGameOfficial @emailAddress =	'lbirdland@gmail.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'WSU Stewart Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Head Linesman'	;
execute usp_addGameOfficial @emailAddress =	'reidcarlos23@gmail.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'WSU Stewart Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Line Judge'	;
execute usp_addGameOfficial @emailAddress =	'tomdeelstra@utah.gov'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'WSU Stewart Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'BackJudge'	;
execute usp_addGameOfficial @emailAddress =	'burnettdan@hotmail.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'Fremont High School'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Referee'	;
execute usp_addGameOfficial @emailAddress =	'football98@gmail.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'Fremont High School'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Umpire'	;
execute usp_addGameOfficial @emailAddress =	'jonathankilburn@aol.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'Fremont High School'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Head Linesman'	;
...
-------------------------------------------------------------------------------------------------------------------------------------------------------
----------Create GameOfficialPaymentHistoryView
---------------------------------------------------------------------------------------------------------------------------------------------------------
if exists(
...
as
	select FirstName, LastName, HTT.GameDateTime, AwayTeamName, HomeTeamName, Sport, [Level], PositionName, AmountPaid
from
	(select FirstName, LastName, GAM.odGame_id, HomeTeam, SchoolName as HomeTeamName, GameDateTime, PositionName
	from gosUser UTT
	inner join gosGameOfficial GGO
	on GGO.odUser_id = UTT.odUser_id
	inner join gosGame GAM
	on GGO.odGame_id = GAM.odGame_id
	inner join gosSchool SCHOO
	on SCHOO.odSchool_id = GAM.HomeTeam
	inner join gosOfficiatingPosition OPP
	on OPP.odPositionName_id = GGO.odPositionName_id) HTT
	inner join
	(select SCH.SchoolName as AwayTeamName, GG.odGame_id, GameDateTime,Sport, [Level], PayRate as AmountPaid
	from gosSchool SCH
	inner join gosGame GG
	on GG.AwayTeam = SCH.odSchool_id
	inner join gosSportLevel
	on gosSportLevel.odSportLevel_id = GG.odSportLevel_id) VT
	on VT.odGame_id = HTT.odGame_id;
...
------------------------------------------------------------------------------------------------------------------------------------------------------------
-------------------Create OfficialGameView view
-----------------------------------------------------------------------------------------------------------------------------------------------------

if exists(
	select * from sysobjects where id = OBJECT_ID('dbo.udv_OfficialGameView')
	)
	drop view dbo.udv_OfficialGameView
	go
	
create view dbo.udv_OfficialGameView
as  

select HTT.odGame_id, HTT.GameDateTime, AwayTeamName, HomeTeamName, Sport, [Level], PositionName, Concat( FirstName,' ', LastName) as Official
from
	(select FirstName, LastName, GAM.odGame_id, HomeTeam, SchoolName as HomeTeamName, GameDateTime, PositionName
	from gosUser UTT
	inner join gosGameOfficial GGO
...
-------official has been assigned to work a game but does not have an assignment for another game at the same time)
------------------------------------------------
Select('These are Valid Conditions')  ----These are the last 2 official assignments for playoff games 
execute usp_addGameOfficial @emailAddress =	'jacksonthomas@yahoo.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'Fremont High School'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Line Judge'	;
execute usp_addGameOfficial @emailAddress =	'brfootball67@gmail.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'Fremont High School'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'BackJudge'	;


-------------------------------------------------------
-----------------Test Invalid Conditions
--------------------------------------------------
select('These are invalid conditions')
--------------------------------------------------------------
-----Official is assigned a game in the same sport at the same time
--------------------------------------------------------------
execute usp_addGameOfficial @emailAddress =	'jacksonthomas@yahoo.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'USU Maverik Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@PositionName=	'Referee'	;
--------------------------------------------------------------------
------Official is assigned a game in a different sport at the same time
------------------------------------------------------------------------
execute usp_addGameOfficial @emailAddress =	'jacksonthomas@yahoo.com'	,@GameDateTime=	'Oct 20, 2021 4:00 PM'	,@GameSite=	'USU Maverik Stadium'	,@Sport =	'Volleyball'	,@Level=	'Varsity'	,@PositionName=	'Referee'	;


----------------------------------------------------------------
------ Test AddGameOfficialPayment and pay 10 officials
-----------------------------------------------------------
execute usp_addGameOfficialPayment @emailAddress =	'adambaxter@yahoo.com'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Northridge High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Referee'	;
execute usp_addGameOfficialPayment @emailAddress =	'Sheldonb@hotmail.com'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Northridge High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Umpire'	;
execute usp_addGameOfficialPayment @emailAddress =	'lbirdland@gmail.com'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Northridge High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Head Linesman'	;
execute usp_addGameOfficialPayment @emailAddress =	'marvdeyoung@outlook.com'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Northridge High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Line Judge'	;
execute usp_addGameOfficialPayment @emailAddress =	'john.eliason@gmail.com'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Northridge High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Back Judge'	;
execute usp_addGameOfficialPayment @emailAddress =	'shawnf@gmail.com'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Fremont High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Referee'	;
execute usp_addGameOfficialPayment @emailAddress =	'francis.morgan@gmail.com'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Fremont High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Umpire'	;
execute usp_addGameOfficialPayment @emailAddress =	'gardner1@comcast.net'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Fremont High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Head Linesman'	;
execute usp_addGameOfficialPayment @emailAddress =	'david.hansen0000@hotmail.com'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Fremont High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Line Judge'	;
execute usp_addGameOfficialPayment @emailAddress =	'dennish@msn.com'	,@Datetime=	'Aug 27 2021 7:00 PM'	,@SiteName=	'Fremont High School'	,@Sport = 	'Football'	,@Level =	'Varsity'	,@Position=	'Back Judge'	;

---------Pay Adam Baxter for an extra game to verify fees are cumulative
execute usp_addGameOfficialPayment @emailAddress =	'adambaxter@yahoo.com'	,@Datetime=	'Oct 20, 2021 4:00 PM'	,@SiteName=	'USU Maverik Stadium'	,@Sport =	'Football'	,@Level=	'Varsity'	,@Position=	'Referee'	;

go
---------------------------------------------------------------
-----------User and Official Payments
-------------------------------------------------------
select('These are Payments to Officials')
Select FirstName, LastName, FeesAccumulated
from gosUser
inner join gosOfficial
on gosUser.odUser_id = gosOfficial.odUser_id;

-------------------------------------------------------------
--------History View ( A view that details the payment history for officials, and what they were paid for each game)
---------------------------------------------------------------------
...
------------------------------------------------------------------
-------- Game View ( A view that displays which officials are working each Game)
------------------------------------------------------------------------
Select('These are records from Game View')
